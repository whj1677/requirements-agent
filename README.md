...
Python 核心、中文本地 Web 工作台，面向已有平台的增量需求。当前六步工作流、真实模型验证及限制见 [PRODUCT-01](docs/PRODUCT-01.md)。原始开发规格保留在 `requirements-agent-codex-kit-v1.1/`，旧验收报告保留各自历史边界。
...
首次使用需要本机 Python 3.11+、Node.js/npm，并运行安装脚本；单机包不包含运行时或模型密钥。已有 `.env` 时保留原文件，不重新创建。打开 `http://127.0.0.1:8765`，使用启动脚本显示的本机访问令牌登录。此令牌仅用于本机登录，不是模型 API Key。临时跳过登录可用 `.\scripts\start.ps1 -NoAuth`（本机无令牌模式，待硬件绑定方案替换；仅监听 loopback，去掉 `-NoAuth` 即恢复标准随机令牌）。已有服务时不要覆盖其dist或数据；在独立解压目录安装，并用 `-Port 8768` 等空闲端口启动。
...
4. 第2步“核对现状、价值与改动范围”：修改助手建议，明确目标使用者、价值、优先级、新增/修改、保持和不涉及范围，勾选本期独立需求编号。可选的“按此方向继续讨论”只记录方向，不连带采纳条目。
5. 第3步“澄清流程与规则”：明确采纳本期内容，逐需求核对角色、入口、流程、数据、权限、结果、异常和关联验收。保存回答不调用模型；“根据回答更新理解”另行核对并运行。未决不能假装已解决；明确移出本期须填写理由。
6. 第4步“核对功能草图”：对照原页面核对局部新增区域、保持项和行为。默认静态低保真草图，不代表最终UI设计；无界面变化时明确说明不适用。阶段核对不代表业务正式批准。
7. 第5步“评审 MRD 与 PRD”：默认分别生成两份同源评审稿，关注点不同。规范条款引用已选底稿原文和独立版本，章节参考两份Word内容维度。当前未澄清或产物过期时不能下载，旧文档只读，切换文档不调用模型。
8. 第6步“确认与交接”：审查当前内容，查看具体阻塞，核对指定版本并提交服务端确认。正式确认绑定版本、范围与哈希；成功后生成指定基线交接包。合成测试点击确认不代表真人业务批准。
