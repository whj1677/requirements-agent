...
首次使用请先运行安装脚本；已有 `.env` 时保留原文件，不重新复制。打开 `http://127.0.0.1:8765`，使用启动脚本显示的本机访问令牌登录。此令牌仅用于本机登录，不是模型 API Key。临时跳过登录可用 `.\scripts\start.ps1 -NoAuth`（本机无令牌模式，待硬件绑定方案替换；仅监听 loopback，去掉 `-NoAuth` 即恢复标准随机令牌）。
