...
2. 在“模型设置”查看密钥配置状态、来源、变量名与接收端。默认 DeepSeek Flash 自动读取项目根目录 `.env` 的 `RA_DEEPSEEK_API_KEY`；修改 `.env` 后重启应用即可生效。也可设置本次进程临时 Key，或继续使用进程环境变量。优先级为临时 Key、进程环境变量、`.env`。更换服务方请在模型设置中指定该服务专用变量名和接收端，防止密钥跨接收端。
