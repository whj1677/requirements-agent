...
正常工作台浏览器入口复现并修正四项体验问题：项目重命名清空输入时弹窗误关闭；先打开方案预览再选择方向后，原型操作未及时反映新方向；资料抽屉中的重试错误提示可能被工作台横幅遮挡；网址资料用途被固定为“竞品参考”。修正后重命名空值保持对话框并禁用保存，预览随方向状态更新，错误在资料抽屉内可见，网址保留所选用途（旧客户端仍默认 reference）。

同轮修正了已复现的 PRD 产物错误：语义引用错误现在能报告具体阻塞引用；截断输出按预算策略升级处理；PRD规范引用提示及章节 ID schema 同步调整。没有触发真实模型请求。

验证：`scripts/check_browser.py`（退出码 0；含重命名空值、确认门禁和旧文档快照路径）；`scripts/check_ui02_browser.py`（退出码 0；两类合成场景）；`npm run build`（退出码 0）；`.venv\\Scripts\\python.exe -m pytest tests -q`（125 passed，退出码 0）。截图及本机运行证据保留在忽略目录，不提交。
