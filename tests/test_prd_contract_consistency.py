...
def test_schema_failures_and_shared_example(tmp_path):
    ...
    assert contract['example']['sections'][0]['narration']==''
    ...
    assert '根节点' in repair and '章节级必填字符串' in repair
    bad=plan();bad['sections']*=3
    for s in bad['sections']:s.pop('narration',None)
    with pytest.raises(Problem) as e:compile_plan(bad,p,'prd')
    assert all(f'sections[{n}]' in e.value.message for n in range(3))
    assert '所有 sections 元素' in repair_instruction(e.value,p)
    for value in (None,42):
        bad=plan();bad['sections'][0]['narration']=value
...
def test_empty_normative_whitelist_preserves_draft_unknowns(tmp_path):
    ...
    r=compile_plan(contract['example'],p,'prd')
...
def test_http_actual_contract_and_targeted_repair(case,shape):
    ...
    def response(value,body,ctx):
        ...
        if len(calls)==1:
            ...
            if shape=='missing':
                ...
                for s in value['sections']:del s['narration']
    ...
    assert [b['max_tokens'] for b in calls]==[6000,6000]
...
def test_only_truncation_requests_compression(tmp_path):
    ...
    assert '压缩叙述' in repair and '全部必填字段' in repair and '不提高 max_tokens' in repair
